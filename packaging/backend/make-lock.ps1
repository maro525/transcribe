...
function Get-NormalizedLines([string]$Path) {
    # Normalize line endings and drop the timestamp-ish autogenerated comment noise
    # is not needed: pip-compile output is stable given same inputs. Compare raw
    # non-empty lines with CRLF/LF normalized.
    (Get-Content -LiteralPath $Path) | ForEach-Object { $_.TrimEnd() }
}
